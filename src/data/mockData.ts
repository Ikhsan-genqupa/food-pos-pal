import { Product, ProductCategory, Outlet, Stock, Transaction, PaymentMethod } from '@/types';
...
export let products: Product[] = [
  { id: '1', name: 'Nasi Goreng', categoryId: '1', price: 25000, image: nasiGoreng, createdAt: new Date(), isActive: true },
  { id: '2', name: 'Mie Goreng', categoryId: '1', price: 22000, image: mieGoreng, createdAt: new Date(), isActive: true },
  { id: '3', name: 'Ayam Bakar', categoryId: '1', price: 35000, image: ayamBakar, createdAt: new Date(), isActive: true },
  { id: '4', name: 'Sate Ayam', categoryId: '1', price: 30000, image: sateAyam, createdAt: new Date(), isActive: true },
  { id: '5', name: 'Es Teh Manis', categoryId: '2', price: 8000, image: esTeh, createdAt: new Date(), isActive: true },
  { id: '6', name: 'Jus Jeruk', categoryId: '2', price: 15000, image: jusJeruk, createdAt: new Date(), isActive: true },
];
...
export function addProduct(product: Omit<Product, 'id' | 'createdAt'>): Product {
  const newProduct: Product = { ...product, id: Date.now().toString(), createdAt: new Date() };
...
export function updateProduct(id: string, updates: Partial<Product>): boolean {
  ...
  if (index > -1) { products[index] = { ...products[index], ...updates }; return true; }
  return false;
}

export function deleteProduct(productId: string): boolean {
  const index = products.findIndex(p => p.id === productId);
  if (index > -1) { products = products.filter(p => p.id !== productId); stocks = stocks.filter(s => s.productId !== productId); return true; }
  return false;
}

// Outlet functions
export function addOutlet(outlet: Omit<Outlet, 'id' | 'createdAt' | 'isActive'>, password: string): Outlet {
  const newOutlet: Outlet = { ...outlet, id: Date.now().toString(), createdAt: new Date(), isActive: true };
  outlets = [...outlets, newOutlet];
  registerOutletUser(outlet.username, password, newOutlet);
  products.forEach(product => {
    stocks.push({ id: `${product.id}-${newOutlet.id}`, productId: product.id, product, outletId: newOutlet.id, outlet: newOutlet, quantity: 0, lastUpdated: new Date() });
  });
  return newOutlet;
}
...
export function updateOutlet(id: string, updates: Partial<Outlet>): boolean {
  ...
  if (index > -1) { outlets[index] = { ...outlets[index], ...updates }; return true; }
  return false;
}
...
export function updateStock(productId: string, outletId: string, quantity: number): boolean {
  ...
  if (index > -1) { stocks[index] = { ...stocks[index], quantity, lastUpdated: new Date() }; return true; }
  else {
    const product = products.find(p => p.id === productId);
    const outlet = outlets.find(o => o.id === outletId);
    if (product && outlet) { stocks.push({ id: `${productId}-${outletId}`, productId, product, outletId, outlet, quantity, lastUpdated: new Date() }); return true; }
  }
  return false;
}
...
export function addTransaction(transaction: Omit<Transaction, 'id' | 'createdAt'>): Transaction {
  const newTransaction: Transaction = { ...transaction, id: `TRX${String(transactions.length + 1).padStart(3, '0')}`, createdAt: new Date() };
  transactions = [...transactions, newTransaction];
  transaction.items.forEach(item => {
    const stockIndex = stocks.findIndex(s => s.productId === item.productId && s.outletId === transaction.outletId);
    if (stockIndex > -1) { stocks[stockIndex].quantity -= item.quantity; }
  });
...
export function registerOutletUser(username: string, password: string, outlet: Outlet): void { outletUsers.set(username, { password, outlet }); }
export function removeOutletUser(username: string): void { outletUsers.delete(username); }
export function validateOutletUser(username: string, password: string): Outlet | null {
  const user = outletUsers.get(username);
  if (user && user.password === password) return user.outlet;
  return null;
}
export function getOutletUsers(): Map<string, { password: string; outlet: Outlet }> { return outletUsers; }
